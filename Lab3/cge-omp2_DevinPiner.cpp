...
bool ComputeGaussianElimination(float **a, int n)
{
    ...
    #pragma omp parallel shared(a,gmax,gindmax) firstprivate(n,k) private(pivot,i,j,temp,pmax,pindmax,singflag)
    for (k = 0 ; k < n-1 ; k++)
    {
        // we need one on the processes to set gmax
        #pragma omp master
        {
            gmax = 0.0;
        }
        ...
        #pragma omp critical
        {
            if (gmax == 0) singflag = false;
        }

        //Swap rows if necessary
        if (gindmax != k)
        {
            #pragma omp for private(j,temp) schedule(static)
            for (j = k; j < n; j++)
            {
                temp = a[gindmax][j];
                a[gindmax][j] = a[k][j];
        ...
        #pragma omp parallel for private(i,j,temp) schedule(static)
        for (i = k+1 ; i < n; i++)
        {
            temp = pivot*a[i][k];
            for (j = k ; j < n ; j++)
